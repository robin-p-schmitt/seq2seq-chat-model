...
class TestUtils(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.output_tensor = torch.ones(
            self.batch_size, self.output_len
        ).long()
        self.encoder_outputs = torch.ones(
            self.batch_size, self.input_len, self.hidden_size
        )

        self.rnn_encoders = [LSTMEncoder(self.vocab_size, self.hidden_size, 4)]
        ...
        self.trans_encoders = [
            TransformerEncoder(
                self.vocab_size, self.hidden_size, 4, DotProductAttention
            )
        ]
        ...
        for encoder in self.rnn_encoders:
            with self.subTest(encoder=encoder.__class__):
                ...
                self.assertSequenceEqual(
                    encoding.shape,
                    [self.batch_size, self.input_len, self.hidden_size * 2],
                )
    ...
    def test_decoding_rnn(self):
        for decoder in self.rnn_decoders:
            with self.subTest(decoder=decoder.__class__):
                decoding = get_decoding_rnn(
                    self.output_tensor, self.encoder_outputs, decoder
                )

                self.assertSequenceEqual(
                    decoding.shape,
                    [self.batch_size, self.output_len, self.vocab_size],
                )

    def test_decoding_trans(self):
        for decoder in self.trans_decoders:
            with self.subTest(decoder=decoder.__class__):
                decoding = get_decoding_trans(
                    self.output_tensor, self.encoder_outputs, decoder
                )
    ...
    def test_beam_search(self):
        top_seqs, top_probs = decode_beam(
            self.input_tensor,
            self.trans_encoders[0],
            self.trans_decoders[0],
            get_encoding_trans,
            get_decoding_trans,
            self.beam_width,
            sos_index=2,
            num_steps=self.num_decoding_steps,
        )

        self.assertSequenceEqual(
            top_seqs.shape,
            [self.batch_size, self.beam_width, self.num_decoding_steps],
        )
        self.assertSequenceEqual(
            top_probs.shape, [self.batch_size, self.beam_width]
        )
    ...
    def test_split_dataset(self):
        ...
        self.assertEqual(len(train_sampler), 0.8 * len(dataset))
        self.assertEqual(len(val_sampler), 0.1 * len(dataset))
        self.assertEqual(len(test_sampler), 0.1 * len(dataset))
